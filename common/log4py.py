...
class Logger:
    ...
    def __Logger_tracestack(self):
        ...
        self.__Logger_module = stack[-4][0].replace('\\','/')  #re.sub��bug, sub('%f','\\g','asdfasf')�ᱨ��bad group name������˽�'\\'�滻��'/'
    ...
    def __Logger_set_instance_options(self, parser, section, instance):
        ...
        for i in range(len(parser.options(section))):
            ...
            if (option == "format"):
                ...
                if upper(value) == 'None':
                    pass
                elif upper(value) == 'DAILY':
                    instance.set_rotation(ROTATE_DAILY)
                elif upper(value) == 'WEEKLY':
                    instance.set_rotation(ROTATE_WEEKLY)
                elif upper(value) == 'MONTHLY':
                    instance.set_rotation(ROTATE_MONTHLY)
        ...
        if ((os.path.exists(self.__FileAppender_filename)) and (self.__FileAppender_rotation != ROTATE_NONE)):
            ...
            if (self.__FileAppender_rotate(statinfo[8])):
                ...
                try:
                    ...
                    import random
                    rename(target_file, target_file+str(random.randint(1,100000)))
                    rename(self.__FileAppender_filename, target_file)
